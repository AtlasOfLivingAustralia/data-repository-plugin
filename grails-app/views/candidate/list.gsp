...
<div class="body content">
    ...
    <div class="list">
        <table class="table table-bordered table-striped">
            <colgroup><col width="45%"/><col width="7%"/><col width="10%"/><col width="3%"/><col width="35%"/>
            </colgroup>
            <thead>
            <tr>
            ...
            <g:each in="${instanceList}" status="i" var="instance">
                <tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
                    <td><g:link action="show"
                                params="[uid: instance.uid]">${fieldValue(bean: instance, field: "name")}</g:link></td>
                    <td>${fieldValue(bean: instance, field: "uid")}</td>
                    <td>${fieldValue(bean: instance, field: "lifecycle")}</td>
                    <td>${fieldValue(bean: instance.dataProvider, field: "name")}</td>
                    <td>${fieldValue(bean: instance.dataResource, field: "name")}</td>
                </tr>
            </g:each>
            </tbody>
        </table>
